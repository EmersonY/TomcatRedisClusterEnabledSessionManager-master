...
/**
 * Tomcat redis session manager 继承IRequestSessionManager 接口里面是各种方法 Lifecycle Tomcat声明周期
 *
 * @author Ranjith
 * @since 1.0
 */
public class RedisSessionManager extends ManagerBase implements IRequestSessionManager, Lifecycle
{
	 ...
	 *
	 * @param: [id]
	 * @return: org.apache.catalina.Session
	 * @author: caiyf
	 * @date: 2019-9-11
	 */
	@Override
	...
	public void save(Session session, boolean forceSave) throws Exception {
		...
		try {
			...
			// 强力保存 真 redisSession 里面有数据 真 当前sessionId 真 当前Session 是否持久化（是否有数据，正在运行） 当前map session 值和session 值不同
